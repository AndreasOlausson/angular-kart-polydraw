...
export class ConcaveHull {
    ...
    constructor(latLngs: L.LatLngs, maxDistance: number){
        const result       = this.convertLatLngs(latLngs);
        this.points      = result.points;
        this.maxDistance = isFinite(maxDistance) ? maxDistance : (result.maxDistance + 1);
    }
    convertLatLngs(latLngs: L.LatLngs) {

        let longestDistance = 0;
        const convertedPoints = latLngs.map(function(latLng: L.LatLng, $index: number) {
    ...
    private distanceTo(firstLatLng: L.LatLng, secondLatLng: L.LatLng): number {
    ...
    getLatLngs() {
        ...
        let byY = this.points.slice().sort(function sort(a: L.Point, b: L.Point) {
            return a.y - b.y;
        });
    ...
    isIntersecting(latLngs, otherLatLngs) {
    ...
    getAngle(current, previous, next) {
