...
import { Area, Compass, Perimeter, PolyDrawUtil } from "./utils";
import { DrawMode, MarkerPosition } from "./enums";
import { TurfHelper } from "./turf-helper";
import { PolygonUtil } from "./polygon.util";
import { IconFactory } from "./icon-factory";
import { IMarkerPositionIndexMap, IMarkerPositions, MarkerParams, PolygonLayerParams } from "./interface";
...
export class PolyDrawService {
            ...
            this.arrayOfFeatureGroups.forEach(featureGroup => {
                ...
                latlngs.forEach((latlng, index) => {
                    ...
                    if (equals && length === 1) {
                        ...
                        this.removeFeatureGroup(featureGroup);
                        this.addPolygonLayer({latlngs: layer.toGeoJSON(), simplify: false});
                    }
                });
            });
        }
    ...
    addViken(polygon) {
        this.addPolygonLayer({latlngs: polygon, simplify: true});
    }
    ...
    private addPolygon(latlngs: Feature<Polygon | MultiPolygon>, simplify: boolean, noMerge: boolean = false) {
        ...
        if (this.mergePolygons && !noMerge && this.arrayOfFeatureGroups.length > 0 && !this.kinks) {
            ...
            this.addPolygonLayer({latlngs: latlngs, simplify: simplify} );
        }
    }

    //fine
    private addPolygonLayer(params: PolygonLayerParams) {
        let featureGroup: L.FeatureGroup = new L.FeatureGroup();

        const latLngs = params.simplify ? this.turfHelper.getSimplified(params.latlngs, params.dynamicTolerance) : params.latlngs;
        ...
        markerLatlngs.forEach(polygon => {
            polygon.forEach((polyElement: ILatLng[], i: number) => {
                if (i === 0) {
                    if(params.markerParams) {
                        const markerParams: MarkerParams = params.markerParams;
                        this.addMarker(polyElement, featureGroup, markerParams);
                    } else {
                        this.addMarker(polyElement, featureGroup);
                    }
    ...
    private polygonClicked(e: any, poly: Feature<Polygon | MultiPolygon>) {
        if (this.config.modes.attachElbow) {
            ...
            if (poly.geometry.type === "MultiPolygon") {
                ...
                this.addPolygonLayer({latlngs: newPolygon, simplify: false});
            }
        }
    }
    ...
    private merge(latlngs: Feature<Polygon | MultiPolygon>) {
        ...
        if (newArray.length > 0) {
            ...
            this.addPolygonLayer({latlngs: latlngs, simplify: true});
        }
    }
    ...
    private subtract(latlngs: Feature<Polygon | MultiPolygon>) {
        ...
        coords.forEach(value => {
            this.addPolygonLayer({latlngs: this.turfHelper.getMultiPolygon([value]), simplify: true});
        });
    }
    ...
    private validateMarkerPositions(idx: IMarkerPositions): IMarkerPositions {
        if (idx.MenuMarker === idx.DeleteMarker ||
            idx.MenuMarker === idx.InfoMarker ||
            idx.InfoMarker === idx.DeleteMarker) {

        }
        return idx;
    }

    private hasEqualPositions(d: number, m: number, i: number): boolean {
        console.log("hasEqualPositions", d, m, i);
        return (d === m || d === i || i === m);
    }

    private createMarkerPositionIndexesForCompass(latlngs: ILatLng[]): IMarkerPositionIndexMap[] {
        const compassIdx: IMarkerPositionIndexMap[] = [];
        compassIdx.push({
            markerPosition: MarkerPosition.SouthWest,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.SouthWest),
            occupied: false
        })
        compassIdx.push({
            markerPosition: MarkerPosition.South,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.South),
            occupied: false
        })
        compassIdx.push({
            markerPosition: MarkerPosition.SouthEast,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.SouthEast),
            occupied: false
        })
        compassIdx.push({
            markerPosition: MarkerPosition.East,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.East),
            occupied: false
        })
        compassIdx.push({
            markerPosition: MarkerPosition.NorthEast,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.NorthEast),
            occupied: false
        })
        compassIdx.push({
            markerPosition: MarkerPosition.North,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.North),
            occupied: false
        })
        compassIdx.push({
            markerPosition: MarkerPosition.NorthWest,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.NorthWest),
            occupied: false
        })
        compassIdx.push({
            markerPosition: MarkerPosition.West,
            polygonIndex: this.getMarkerIndex(latlngs, MarkerPosition.West),
            occupied: false
        })
        return compassIdx;
    }
    ...
    private addMarker(latlngs: ILatLng[], FeatureGroup: L.FeatureGroup, markerParams?: MarkerParams) {

        const availableIndexes = this.createMarkerPositionIndexesForCompass(latlngs);
        let deleteMarkerIdx = 0;
        let menuMarkerIdx = 0;
        let infoMarkerIdx = 0;

        let deleteMarkerMapping = availableIndexes.find(value => value.markerPosition === this.config.markers.markerDeleteIcon.position);
        if (deleteMarkerMapping) {
            deleteMarkerIdx = deleteMarkerMapping.polygonIndex;
            availableIndexes.find(value => value.markerPosition === deleteMarkerMapping.markerPosition).occupied = true;
        }
        let menuMarkerMapping = availableIndexes.find(value => value.markerPosition === this.config.markers.markerMenuIcon.position);
        if (menuMarkerMapping) {
            let offsetClockwise = true;
            menuMarkerIdx = menuMarkerMapping.polygonIndex;
            let markerPosition = menuMarkerMapping.markerPosition;
            let newPosition = 0;
            if (deleteMarkerIdx === menuMarkerIdx) {
                if (this.config.markers.markerMenuIcon.position <= this.config.markers.markerDeleteIcon.position) {
                    offsetClockwise = false;
                }
                let equal = true;
                while (equal) {
                   if(offsetClockwise) {
                    ...
                    menuMarkerIdx = availableIndexes.find(value => value.markerPosition === markerPosition).polygonIndex;
                    if(deleteMarkerIdx !== menuMarkerIdx) {
                        equal = false;
                    }
                }
        ...
        let infoMarkerMapping = availableIndexes.find(value => value.markerPosition === this.config.markers.markerInfoIcon.position);
        if (infoMarkerMapping) {
            let offsetClockwise = true;
            infoMarkerIdx = infoMarkerMapping.polygonIndex;
            let markerPosition = infoMarkerMapping.markerPosition;
            let newPosition = 0;
            if (infoMarkerIdx === menuMarkerIdx || infoMarkerIdx === deleteMarkerIdx) {
                if (this.config.markers.markerInfoIcon.position <= this.config.markers.markerDeleteIcon.position) {
                    offsetClockwise = false;
                }
                let equal = true;
                while (equal) {
                    ...
                    infoMarkerIdx = availableIndexes.find(value => value.markerPosition === markerPosition).polygonIndex;
                    if(infoMarkerIdx !== menuMarkerIdx && deleteMarkerIdx !== infoMarkerIdx) {
                        equal = false;
                    }
                }
            }
        ...
        latlngs.forEach((latlng, i) => {
            ...
            //TODO Add modulo smartness... zoomlevel? marker count?
            let markerParamClass = "";
            if(markerParams) {
                if(!ignoreMarkerParams){
                    if(i%this.config.tools.bezier.markerParams.hiddenModuloLevel !== 0) {
                        markerParamClass = markerParams.markerClass;
                    }
                }
            }
            ...
            const marker = new L.Marker(latlng, {
                icon: IconFactory.createDivIcon(iconClasses, markerParamClass),
    ...
    private unionPolygons(layers, latlngs: Feature<Polygon | MultiPolygon>, polygonFeature) {
        ...
        this.addPolygonLayer({latlngs: newLatlngs, simplify: true});
    }
    ...
    private convertToBoundsPolygon(latlngs: ILatLng[]) {
        ...
        this.addPolygonLayer({latlngs: this.turfHelper.getTurfPolygon(newPolygon), simplify: false});
    }
    ...
    private doubleElbows(latlngs: ILatLng[]) {
        ...
        this.addPolygonLayer({latlngs: this.turfHelper.getTurfPolygon(newPolygon), simplify: false, dynamicTolerance: false});
    }

    //TODO implement hideRandomMarkers (only show one out of ten or something like that, depending on markers count? percentage?)
    private bezierify(latlngs: ILatLng[]) {
        ...
        const markerParams: MarkerParams = {
            hiddenModuloLevel: this.config.tools.bezier.markerParams.hiddenModuloLevel,
            hidden: true,
            markerClass: this.config.tools.bezier.markerParams.markerClass
        }
        this.addPolygonLayer({latlngs: this.turfHelper.getTurfPolygon(newPolygon), simplify: false, dynamicTolerance: false, markerParams});
    }

    private getMarkerIndex(latlngs: ILatLng[], position: MarkerPosition, offsetClockwise: number = 0): number {
        ...
        return this.turfHelper.getNearestPointIndex(targetPoint, fc as any);
    }
    //TODO Config show/hide tools
